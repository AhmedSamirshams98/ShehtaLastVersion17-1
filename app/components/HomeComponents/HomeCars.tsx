...
interface HomeCarsProps {
  showAll?: boolean; // true = كل السيارات، false = السيارات المتاحة فقط
}
...
const HomeCars: React.FC<HomeCarsProps> = ({ showAll = false }) => {
  ...
  const fetchCars = async () => {
    ...
    setLoading(true);
    try {
      const endpoint = showAll ? "/api/cars" : "/api/cars/available";
  ...
  const OPTIONS = { loop: false, duration: 20 };
  ...
  const getSelectedBrandDisplayName = () =>
    selectedBrand === "all" ? "كل السيارات" : selectedBrand;

  // إنشاء Slides لكل سيارة
  const carSlides = filteredCars.map((car) => {
    const slides = car.car_images.map((img, index) => (
      <div key={index} className="slider-slide relative object-cover">
        <Image
          src={img.image_url}
          fill
          alt={`${car.brand} ${car.model}`}
  ...
  if (hasMore) {
    carSlides.push(
      <div
        key="load-more"
        className="h-full flex flex-col items-center justify-center p-6 rounded-[26px] border-yellow-400 transition-transform hover:scale-105"
      >
        <button
          onClick={fetchCars}
          disabled={loading}
          className="flex items-center gap-3 bg-yellow-500 hover:bg-yellow-400 text-black font-bold text-[4vw] md:text-[1.5vw] lg:text-[1vw] px-6 py-4 rounded-2xl shadow-md transition-all"
        >
          {loading ? (
            <>
              <svg
  ...
  return (
    ...
    >
      <div className="px-[6%] lg:px-0 mt-4 w-full md:w-[70%] flex flex-row justify-between items-center gap-4">
        <h1 className="text-[4vw] md:text-[3vw] lg:text-[1.4vw] font-bold text-center md:text-right text-gray-900">
          {showAll ? "كل المعروضات" : "أحدث المعروضات ✨"}
        </h1>
      ...
      <div className="w-full relative flex flex-col items-center justify-center gap-4">
        {filteredCars.length > 0 ? (
          <EmblaCarousel slides={carSlides} options={OPTIONS} />
        ) : (
          <div className="text-center py-8">
            <p className="text-gray-500">لا توجد سيارات لعرضها</p>
          </div>
